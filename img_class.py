...
Model Details:
    Architecture:                   Conv2d, 4*DWC Block
    Total Parameters:               ~257,000 (255,434 Trainable Params)
    Train Time:                     ~2-3 min per epoch           
    
    *Hyperparams tuning may cause flucuations in accuracies. 
    
    Train | Test | Validation split  = 60 | 10 | 30
...
with strategy.scope():
    ...
    model = tf.keras.Sequential([
      ...
      tf.keras.layers.DepthwiseConv2D(kernel_size=3, strides=1, padding='same', activation='relu'),
      ...
      # Depthwise Convolution Block 4
      tf.keras.layers.DepthwiseConv2D(kernel_size=3, strides=1, padding='same', activation='relu'),
      tf.keras.layers.Conv2D(filters=256, kernel_size=2, strides=1, padding='same', activation='relu'),
      tf.keras.layers.MaxPool2D(),
